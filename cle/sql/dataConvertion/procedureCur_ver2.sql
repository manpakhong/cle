drop procedure if exists importResource;
CREATE PROCEDURE importResource()
BLOCK1: begin
  ...
  declare _ScreenCap varchar(255);

  DECLARE curOrig CURSOR FOR SELECT * FROM vwrallresources;

  DECLARE CONTINUE HANDLER FOR NOT FOUND SET done_block1 = 1;
  delete from cle_resource;
  OPEN curOrig;


  origLoop: LOOP
    FETCH curOrig INTO _ResourcesID, _CMS_ID, _URL, _ResourceName, _author, _TeachingAims, _Lv1, _Lv2, _Lv3, _Lv4, _Lv5, _Lv6, _Lv7, _Lv1_Text, _Lv2_Text, _Lv3_Text, _Lv4_Text, _Lv5_Text, _Lv6_Text, _Lv7_Text, _ScreenCap;
    ...
    */
    ...
    typeLoop: LOOP
        FETCH curType INTO _sid, _Seq, _LV, _LV_Text, _UpLV_sid, _IsShown, _IsNetvigated, _urlv, _Remarks, _LastUpdate;
    ...
    /* select 'loop2';*/
    if rtrim(ltrim(_LV7_Text)) != '-' then
      if rtrim(ltrim(_LV7_Text)) = _LV_Text then
        insert into cle_resource (URL, ResourceName, Author, TeachingAims, TypeMenu_sid, Image_URL)
        values(_URL, _ResourceName, _author, _TeachingAims, _sid, _ScreenCap);
      end if;
    else /* lv7 else */

    if rtrim(ltrim(_LV6_Text)) != '-' then
      if rtrim(ltrim(_LV6_Text)) = _LV_Text then
        insert into cle_resource (URL, ResourceName, Author, TeachingAims, TypeMenu_sid, Image_URL)
        values(_URL, _ResourceName, _author, _TeachingAims, _sid, _ScreenCap);
      end if;
    else /* lv6 else */
    ...
    end if; /* lv6 end if ...else */
    end if; /* lv7 end if ...else */
    /*
