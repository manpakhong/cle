<?php
	...
	class EcrUserFilter extends EcrUser
	{
		private $orderByList;
		...
		public function getWhereClause($_alias = '')
		{
			...
			foreach ($arr as $key => $value) {
			    ...
			    if (!is_null($value))
			    {
			    	...
			    	if ($_alias == '')
			    	{
			    		$returnWhereClause = $returnWhereClause . EcrUser::getDbFieldName($key) . "='" . $value . "'";
			    	}
			    	else
			    	{
			    		$returnWhereClause = $returnWhereClause . $_alias. '.'. EcrUser::getDbFieldName($key) . "='" . $value . "'";			    		
			    	}
			    }
			}
